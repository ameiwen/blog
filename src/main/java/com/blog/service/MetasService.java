...
public class MetasService {
    ...
    private List<Contents> getMetaContents(Metas m) {
        ...
        List<Contents> contents = new Contents().queryAll("select * from bl_contents where cid in (" + cidList.stream().map(Object::toString).collect(Collectors.joining(",")) + ") order by created desc");
        return contents;
    }
