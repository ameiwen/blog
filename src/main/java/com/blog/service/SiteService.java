...
import com.blog.init.SqliteJdbc;
...
public class SiteService {
    ...
    public List<Contents> getContens(String type, int limit) {
        ...
        if (Types.RANDOM_ARTICLE.equals(type)) {
            List<Integer> cids = new ActiveRecord().queryAll(Integer.class, "select cid from bl_contents where type = ? and status = ? order by random() * cid limit ?", Types.ARTICLE, Types.PUBLISH, limit);
    ...
    public List<Archive> getArchives() {
        String sql = "select FROM_UNIXTIME(created,'%Y年%m月') as date_str, count(*) as count  from bl_contents " +
    ...
    public Contents getNhContent(String type, Integer created) {
        ...
        if (Types.NEXT.equals(type)) {
            contents = new Contents().query("SELECT * FROM bl_contents WHERE type = ? AND status = ? AND created > ? ORDER BY created ASC LIMIT 1", Types.ARTICLE, Types.PUBLISH, created);
        }
        if (Types.PREV.equals(type)) {
            contents = new Contents().query("SELECT * FROM bl_contents WHERE type = ? AND status = ? AND created < ? ORDER BY created DESC LIMIT 1", Types.ARTICLE, Types.PUBLISH, created);
        }
        return contents;
    }
